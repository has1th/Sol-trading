...
export async function makeSwap(poolKeys, amountIn, minAmountOut, reverse: Boolean, wallet) {
	...
	if (reverse === false) {
 	transaction.push(UNITLIMIT);
	transaction.push(UNITPRICE);
 	transaction.push(createWsolQuoteAta);
	transaction.push(SystemProgram.transfer({fromPubkey: wallet.publicKey, toPubkey: poolKeys.ownerQuoteAta, lamports: amountIn }), spl.createSyncNativeInstruction(poolKeys.ownerQuoteAta)); // 10000000 lamports will send 0.01 sol to the ata
	transaction.push(createTokenBaseAta)
	transaction.push(swap)
	transaction.push(closeSol)
 }
	if (reverse === true) {
 	transaction.push(UNITLIMIT);
 	transaction.push(UNITPRICE);
